 ...
 * alternative, `zapo` = the linked-device engine with real-time voice calls.
 *
 ...
 *
 * `evidence` (present only when at least one adapter is `not-available`) cites inspected library
 * symbols, or the explicit adapter throw for Zapo methods not claimed in its first slice.
 *
 ...
 * wwjs/Baileys `rootCause`/`evidence` fields are hand-curated, source-traced annotations; Zapo gaps
 * stay `uncertain` until similarly traced. They are NOT regenerated by the spec.
 *
 ...
 * wwjs catalog reads (getCatalog/getProducts/getProduct) are marked `not-available` even though their
 * adapter bodies do not throw. If the drift gate is extended to assert against this matrix, it must
 * consult `status`, not just the throw pattern, for these rows (or the adapter stubs must start
 * throwing).
 */
...
export interface MethodCapability {
  ...
  zapo: AdapterCapability;
  /** Cited library/adapter symbols. Present only when at least one adapter is not-available. */
  evidence?: string;
}

type LegacyMethodCapability = Omit<MethodCapability, 'zapo'>;

const LEGACY_ENGINE_CAPABILITY_MATRIX: Record<string, LegacyMethodCapability> = {
...
/**
 * Zapo's first built-in slice intentionally exposes the lifecycle/status methods, plain text,
 * number resolution, basic chat mutations, and the complete separate IVoiceCallEngine contract.
 * Every other IWhatsAppEngine method rejects with EngineNotSupportedError. `uncertain` is deliberate:
 * those methods were not claimed from undocumented/raw Zapo APIs, so their underlying-library
 * feasibility needs a declaration/live spike before being called an adapter gap or library limit.
 */
const ZAPO_SUPPORTED_METHODS = new Set([
  'initialize',
  'disconnect',
  'logout',
  'destroy',
...
export const ENGINE_CAPABILITY_MATRIX: Record<string, MethodCapability> = Object.fromEntries(
  Object.entries(LEGACY_ENGINE_CAPABILITY_MATRIX).map(([method, capability]) => {
    const zapoSupported = ZAPO_SUPPORTED_METHODS.has(method);
    return [
      method,
      {
        ...capability,
        zapo: zapoSupported ? { status: 'supported' } : { status: 'not-available', rootCause: 'uncertain' },
        ...(!zapoSupported
          ? {
              evidence: [capability.evidence, `ZapoAdapter.${method} rejects with EngineNotSupportedError`]
                .filter(Boolean)
                .join('; '),
            }
          : {}),
