...
This page is the operator- and engineer-facing view of which `IWhatsAppEngine` capabilities are real on each adapter — `wwjs` (whatsapp-web.js, the default), `baileys` (the browser-free alternative), and `zapo` (the linked-device engine with real-time voice calls) — and, for the ones that are missing, _why_ and _where to start_.

The committed source of truth is `src/engine/engine-capability-matrix.ts`. Every `IWhatsAppEngine` method has a row with all three adapters; each adapter is either `supported` (works end-to-end) or `not-available`, and each `not-available` cell carries a **rootCause**:
...
Each unavailable row also cites an **evidence** string: an inspected library symbol where source-traced, or the explicit adapter throw for Zapo's intentionally unclaimed first slice.
...
The wwjs/Baileys `rootCause`/`evidence` fields are hand-curated from installed-library source traces. Zapo's unavailable cells remain `uncertain` until equivalent declaration/live tracing is done; the parity spec verifies that every one still throws explicitly.
...
## Zapo first-slice capability

Zapo supports these 20 `IWhatsAppEngine` methods:

- Lifecycle/status: `initialize`, `disconnect`, `logout`, `destroy`, `forceDestroy`, `getStatus`, `getQRCode`, `requestPairingCode`, `getPhoneNumber`, `getPushName`
- Messaging/identity: `sendTextMessage`, `checkNumberExists`, `getNumberId`, `resolveContactPhone`
- Chat reads/state: `getChats`, `getChatHistory`, `sendSeen`, `markUnread`, `deleteChat`, `sendChatState`

Every other `IWhatsAppEngine` method rejects with `EngineNotSupportedError` (HTTP 501); the matrix marks those Zapo cells `not-available — uncertain` because no undocumented/raw Zapo API is claimed without a declaration trace and live verification.

Voice calling is a separate optional contract, `IVoiceCallEngine`, and is fully implemented by Zapo: start/accept/reject/end, mute, external live audio, 16 kHz mono PCM feed and watermarks, call snapshots, and `incoming`/`state`/`ended`/`audio`/`error` events. The built-in uses `zapo-js` 1.5.0, `@zapo-js/store-sqlite` 1.0.2, and `@zapo-js/voip` 1.0.0. SQLite state lives at `<ZAPO_AUTH_DIR>/<session-name>/state.sqlite`. Proxy settings are currently ignored.

## Legacy adapter unwired-capability inventory

The detailed source traces below cover the pre-existing wwjs/Baileys gaps. Zapo's intentionally narrow first slice is listed separately above.
...
- **`deleteStatus` (baileys) caveat.** Marked `supported` (no throw), but the adapter self-describes its `sendMessage(status@broadcast,{delete})` revoke shape as _empirically unverified_ (`baileys.adapter.ts:909-911`) — only posting was live-spiked. May need a fallback to `EngineNotSupportedError` if WA rejects the shape.
...
- `getContactStatus` / `getContactStatuses` — `fetchStatus` returns the _about_ text, not 24h stories; stories only surface as `status@broadcast` messages. Needs an OpenWA-side story accumulator.
...
- **71** interface methods, **213** adapter-cells (71 × 3 engines).
- **141** supported cells; **72** not-available cells across **53** methods.
- Of the 72 not-available cells: **5 adapter-gaps** (fixable) + **14 library-limitations** + **53 uncertain** (Zapo methods deliberately not claimed in the first slice).
- **3 phantom-support corrections** (the wwjs catalog reads silently stub instead of throwing but are honestly `not-available`).
